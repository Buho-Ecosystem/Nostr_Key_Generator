...
# ====== Option 5 (NIP-49) deps: stdlib + PyNaCl only ======
import getpass
import unicodedata
try:
    # PyNaCl (libsodium) AEAD XChaCha20-Poly1305 (IETF)
    from nacl.bindings import crypto_aead_xchacha20poly1305_ietf_decrypt
except Exception:  # PyNaCl not installed or too old
    crypto_aead_xchacha20poly1305_ietf_decrypt = None  # type: ignore
...
# =========================
# Option 5: nsec.app Decrypter (NIP-49, PyNaCl)
# =========================

def _ncryptsec_to_raw(bech: str) -> bytes:
    """Decode bech32 ncryptsec -> raw 8-bit bytes."""
    hrp, data5 = _bech32_decode(bech)
    if hrp != 'ncryptsec':
        raise ValueError("Input is not an ncryptsec… string")
    raw = bytes(_convertbits(data5, 5, 8, pad=False))
    if len(raw) < 59:  # minimal sanity (nonce/tag sizes etc); spec says 91 bytes typical
        raise ValueError("ncryptsec payload too short")
    return raw


def decrypt_ncryptsec_to_privhex(ncryptsec: str, password: str) -> str:
    """
    Decrypt NIP-49 ncryptsec using scrypt + XChaCha20-Poly1305 (PyNaCl).
    Returns 64-hex private key.
    """
    if crypto_aead_xchacha20poly1305_ietf_decrypt is None:
        raise RuntimeError("PyNaCl not available. Install with: pip install pynacl")

    raw = _ncryptsec_to_raw(ncryptsec)
    # Layout per NIP-49 v0x02:
    # [0] version (0x02)
    # [1] log_n (base-2 exponent for scrypt N)
    # [2:18] salt (16 bytes)
    # [18:42] nonce (24 bytes)
    # [42] key security byte (AAD)
    # [43:] ciphertext (private key 32 bytes + 16-byte tag = 48 bytes)
    ver = raw[0]
    if ver != 0x02:
        raise ValueError(f"Unsupported ncryptsec version: 0x{ver:02x}")
    log_n = raw[1]
    salt = raw[2:18]
    nonce = raw[18:42]
    aad = raw[42:43]
    ...
    if len(salt) != 16 or len(nonce) != 24 or len(ciphertext) < 16:
        raise ValueError("Malformed ncryptsec payload")

    # NFKC-normalize password as per spec
    pw = unicodedata.normalize("NFKC", password).encode("utf-8")

    # scrypt KDF
    n_value = 1 << log_n
    try:
        key = __import__("hashlib").scrypt(password=pw, salt=salt, n=n_value, r=8, p=1, dklen=32)
    except MemoryError as e:
        raise MemoryError(f"scrypt parameters too large (log_n={log_n}) for this system") from e

    # AEAD decrypt (raises on failure)
    plaintext = crypto_aead_xchacha20poly1305_ietf_decrypt(ciphertext, aad, nonce, key)
    if len(plaintext) != 32:
        raise ValueError("Decrypted key has unexpected length")
    return plaintext.hex()


def nsecapp_decrypt_menu():
    hr()
    print(color("nsec.app Decrypter (NIP-49)", S.B))
    enc = prompt("Paste exported encrypted secret (ncryptsec1…):").strip()
    if not enc.lower().startswith("ncryptsec1"):
        print(color("Input does not look like an ncryptsec… string", S.R))
        input(color("\nPress Enter to return…", S.D))
        return
    if crypto_aead_xchacha20poly1305_ietf_decrypt is None:
        print(color("PyNaCl not available. Install with: pip install pynacl", S.R))
        input(color("\nPress Enter to return…", S.D))
        return

    pw = getpass.getpass("Enter your passphrase: ")
    try:
        priv_hex = decrypt_ncryptsec_to_privhex(enc, pw)
        ident = identity_from_priv_hex(priv_hex)
        view_identity(ident)
    except Exception as e:
        print(color(f"Decryption failed: {e}", S.R))
        input(color("\nPress Enter to return…", S.D))
